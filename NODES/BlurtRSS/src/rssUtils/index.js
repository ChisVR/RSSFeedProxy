...
const makeImageUrl = async (username) => {
    ...
    try {
        const account_details = await blurt.api.callAsync('condenser_api.get_accounts', [[username]])
...
const rssGeneratorVoter = async (voter, iface, limit, minVotePct, tagFilter, refer) => {
    ...
    const apiResponse = await blurt.api.callAsync('condenser_api.get_account_history', [voter, -1, 1000])
...
const getDiscussionsByCreated = promisify(blurt.api.getDiscussionsByCreated);
const getDiscussionsByFeed = promisify(blurt.api.getDiscussionsByFeed);
const getDiscussionsByBlog = promisify(blurt.api.getDiscussionsByBlog);
const getDiscussionsByHot = promisify(blurt.api.getDiscussionsByHot);
const getDiscussionsByTrending = promisify(blurt.api.getDiscussionsByTrending);
const getDiscussionsByPromoted = promisify(blurt.api.getDiscussionsByPromoted);
const getDiscussionsByComments = promisify(blurt.api.getDiscussionsByComments);
const getDiscussionsByAuthorBeforeDate = promisify(blurt.api.getDiscussionsByAuthorBeforeDate);
